...
const Booking: React.FC = () => {
  ...
  if (loading && categories.length === 0) return <div className="flex flex-col items-center justify-center h-96"><RefreshCcw className="animate-spin text-indigo-600 mb-4" size={40} /><p className="text-slate-500 font-medium italic">Yuklanmoqda...</p></div>;

  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <UIModal {...modal} onClose={() => setModal({ ...modal, isOpen: false })} />
      
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-zinc-900 p-6 rounded-[2.5rem] border shadow-sm">
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
          {categories.map((cat) => (
            <div key={cat.id} className="relative group shrink-0">
              <button onClick={() => handleCategoryChange(cat.id)} className={`px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest border transition-all ${selectedCategoryId === cat.id ? 'bg-slate-900 text-white dark:bg-indigo-600' : 'bg-white dark:bg-zinc-950 text-slate-500'}`}>{cat.name}</button>
              <div className="absolute -top-2 -right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-all z-10"><button onClick={(e) => handleEditCategory(e, cat)} className="w-7 h-7 bg-white dark:bg-zinc-800 border rounded-full flex items-center justify-center text-indigo-600 shadow-md"><Edit2 size={12} /></button><button onClick={(e) => handleDeleteCategory(e, cat.id)} className="w-7 h-7 bg-white dark:bg-zinc-800 border rounded-full flex items-center justify-center text-red-500 shadow-md"><X size={12} /></button></div>
            </div>
          ))}
        </div>
        <div className="flex gap-3 shrink-0"><button onClick={handleAddCategory} className="px-5 py-3 bg-slate-50 dark:bg-zinc-800 text-slate-600 font-black rounded-2xl text-[11px] uppercase tracking-widest">Toifa +</button><button onClick={handleAddRoom} className="px-6 py-3 bg-indigo-600 text-white font-black rounded-2xl text-[11px] uppercase tracking-widest">Xona +</button></div>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
        {rooms.map((room) => (
          <div key={room.id} onClick={() => toggleRoomStatus(room)} className="group relative bg-white dark:bg-zinc-900 border rounded-[2.5rem] p-8 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all cursor-pointer flex flex-col items-center justify-between min-h-[220px]">
            ...
            <div className="mt-16 w-20 h-20 rounded-3xl bg-slate-50 dark:bg-zinc-950 flex items-center justify-center text-slate-200 group-hover:text-indigo-600 transition-all"><LayoutGrid size={40} /></div>
            ...
            <button onClick={(e) => handleDeleteRoom(e, room.id)} className="absolute top-4 right-4 p-2 bg-white dark:bg-zinc-800 border text-red-400 opacity-0 group-hover:opacity-100 rounded-full transition-all"><Trash2 size={14} /></button>
          </div>
        ))}
        {rooms.length === 0 && selectedCategoryId && <div className="col-span-full py-32 text-center bg-white/50 dark:bg-zinc-900/50 rounded-[3rem] border-2 border-dashed"><h3 className="text-xl font-black text-slate-400 uppercase tracking-tighter mb-2">Ushbu toifada hali stollar yo'q</h3><button onClick={handleAddRoom} className="px-8 py-3 bg-indigo-600 text-white font-black rounded-2xl text-xs uppercase tracking-widest">Birinchi stolni qo'shing</button></div>}
      </div>
    </div>
  );
};
