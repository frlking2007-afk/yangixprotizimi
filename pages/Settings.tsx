...
const Settings: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      {/* Profile Header */}
      <div className="bg-white dark:bg-zinc-800 hacker:bg-black hacker:border hacker:border-[#0f0] rounded-[2.5rem] p-6 md:p-8 border border-slate-100 shadow-sm flex flex-col md:flex-row items-center gap-6 md:gap-8">
        <div className="w-20 h-20 md:w-24 md:h-24 rounded-3xl bg-indigo-50 dark:bg-zinc-700 hacker:bg-[#001100] hacker:border hacker:border-[#0f0] flex items-center justify-center text-indigo-600 dark:text-indigo-400 hacker:text-[#0f0] relative shrink-0">
          <User size={40} />
          ...
          {isEditing ? (
            <div className="flex flex-col md:flex-row items-center gap-3">
              <input 
                ...
                className="px-4 py-2 bg-slate-50 dark:bg-zinc-900 hacker:bg-black hacker:border hacker:border-[#0f0] border border-slate-100 rounded-xl outline-none font-bold text-xl dark:text-white"
                autoFocus
              />
            ...
            <div className="flex flex-col md:flex-row items-center gap-4">
              ...
              <button onClick={() => setIsEditing(true)} className="p-2.5 bg-slate-50 dark:bg-zinc-700 text-slate-400 hover:text-indigo-600 rounded-xl"><Edit3 size={18} /></button>
            </div>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-zinc-800 hacker:bg-black hacker:border hacker:border-[#0f0] rounded-[2rem] p-6 border border-slate-100 shadow-sm">
            <form onSubmit={handleUpdatePassword} className="space-y-4">
              ...
              <button 
                ...
                className="w-full py-3.5 bg-slate-900 dark:bg-zinc-700 hacker:bg-[#0f0] text-white hacker:text-black font-black rounded-2xl flex items-center justify-center gap-2 hover:bg-slate-800 transition-all shadow-lg shadow-slate-200 dark:shadow-none"
              >
            ...
            {themes.map((t) => (
              <button
                ...
                className={`
                  ...
                  ${currentTheme === t.id ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/20' : 'border-slate-100 dark:border-zinc-800 hover:border-indigo-200'}
                  ${t.color}
                `}
              >
                <div className="flex items-center gap-4">
                  <div className={`p-2.5 rounded-xl ${currentTheme === t.id ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-zinc-800 text-slate-500'}`}>
                    <t.icon size={20} />
                  </div>
